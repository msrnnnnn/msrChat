...
QString AuthController::validateUsername(const QString &u) const
{
    return u.trimmed().isEmpty() ? QStringLiteral("用户名不能为空") : QString();
}
...
QString AuthController::validateVerifyCode(const QString &c) const
{
    return c.trimmed().isEmpty() ? QStringLiteral("验证码不能为空") : QString();
}

void AuthController::login(const QString &username, const QString &password)
{
    QString err = validateUsername(username);
    if (!err.isEmpty())
    {
    ...
    _is_logging_in = true;
    emit sigIsLoggingInChanged();
    LoginReqStruct req;
    req.user = username;
    req.passwd = Utils::hashPassword(password);
...
void AuthController::slotLoginRsp(const LoginRspStruct &rsp)
{
    ...
    ChatLoginReqStruct chatReq;
    chatReq.uid = _uid;
    chatReq.token = _token;
...
{
    QString err = validateEmail(email);
    if (!err.isEmpty())
    {
        emit verifyCodeResult(false, err);
        return;
    }
    VerifyCodeReqStruct req;
    req.email = email.trimmed();
                                  ...
                                  const QString &confirmPassword, const QString &verifyCode)
{
    ...
    err = validateVerifyCode(verifyCode);
    if (!err.isEmpty())
    {
        emit registerResult(false, err);
        return;
    ...
    req.email = email.trimmed();
    req.passwd = Utils::hashPassword(password);
    req.verifycode = verifyCode.trimmed();
...
void AuthController::slotVerifyCodeRsp(const VerifyCodeRspStruct &rsp)
{
    if (rsp.error != static_cast<int>(ERRORCODES::SUCCESS))
    {
        ...
        emit verifyCodeResult(false, errStr);
        return;
    }
...
void AuthController::slotRegisterRsp(const RegisterRspStruct &rsp)
{
    if (rsp.error != static_cast<int>(ERRORCODES::SUCCESS))
    {
        ...
        emit registerResult(false, errStr);
        return;
    }
    emit registerResult(true, tr("注册成功！"));
}

void AuthController::resetPassword(const QString &username, const QString &email, const QString &newPassword,
                                   const QString &verifyCode)
{
    ...
    if (!err.isEmpty())
    {
        emit resetPasswordResult(false, err);
        return;
    }
    err = validateEmail(email);
    if (!err.isEmpty())
    ...
    req.user = username;
    req.email = email.trimmed();
    req.passwd = Utils::hashPassword(newPassword);
    req.verifycode = verifyCode.trimmed();
...
void AuthController::slotResetPwdRsp(const ResetPwdRspStruct &rsp)
{
    if (rsp.error != static_cast<int>(ERRORCODES::SUCCESS))
    {
        ...
        emit resetPasswordResult(false, errStr);
        return;
    }
