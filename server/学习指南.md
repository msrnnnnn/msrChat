...
### 客户端改动
- 注册提交密码与确认密码改为异或编码发送，注册回包只处理注册模块并统一错误提示
- 登录页完善错误提示、回包分发与“忘记密码”入口
- 新增重置密码对话框（UI 与逻辑），支持验证码获取与重置请求
- 主窗口新增登录/注册/重置三页切换
- 客户端错误码补齐并与服务端对齐

### 服务端改动
- GateServer 新增 /reset_pwd 与 /user_login 路由
- MySQL DAO 增加重置密码与登录查询接口
- 新增 StatusGrpcClient 与连接池，登录流程接入状态服务
- proto 增加 StatusService 相关消息
- GateServer 配置新增 StatusServer 段

### 相关文件清单
- 客户端：client/QmsrChat/registerdialog.cpp, logindialog.cpp, resetdialog.*,
  mainwindow.*, global.h, CMakeLists.txt
- 服务端：server/GateServer/src/LogicSystem.cpp, src/MysqlDao.cpp, src/MysqlMgr.cpp,
  include/StatusGrpcClient.h, src/StatusGrpcClient.cpp, include/const.h, config.ini,
  CMakeLists.txt
- 协议：shared/message.proto
