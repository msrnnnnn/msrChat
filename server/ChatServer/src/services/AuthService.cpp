...
// ─── HandleDeleteUserRequest (ID_DELETE_USER 3001) ───

bool AuthService::HandleDeleteUserRequest(CSession &session, const std::string &body_data)
{
    DispatchGuard guard(session);
    ...
    try
    {
        ...
        if (uid <= 0 || token.empty())
        {
            nlohmann::json response{{"error", ERR_INVALID_PARAM}, {"message", "参数无效"}};
            session.Send(response.dump(), ID_DELETE_USER);
            return true;
        }

        if (!TokenManager::Instance().CheckToken(uid, token))
        {
            nlohmann::json response{{"error", ERR_PASSWD_ERR}, {"message", "token 无效"}};
            session.Send(response.dump(), ID_DELETE_USER);
            return true;
        }
        ...
        server->GetThreadPool().Enqueue(
            [safe_session, uid]()
            {
                ...
                auto user = SQLiteMgr::Instance().Auth().GetUserByUid(uid);
                if (!user.has_value())
                {
                    nlohmann::json response{{"error", ERR_USER_NOT_EXIST}, {"message", "用户不存在"}};
                    safe_session->Send(response.dump(), ID_DELETE_USER);
                    safe_session->ContinueReading();
                    return;
                }

                std::string deleted_name = "已注销_" + std::to_string(uid);
                SQLiteMgr::Instance().Auth().UpdateUserProfile(uid, deleted_name, "");

                TokenManager::Instance().RemoveToken(uid);

                spdlog::info("[AuthService] User {} deleted (renamed to {})", uid, deleted_name);

                nlohmann::json response{{"error", 0}, {"message", "账户已注销"}};
                safe_session->Send(response.dump(), ID_DELETE_USER);
                safe_session->ContinueReading();
            });
    ...
    {
        spdlog::error("[AuthService] HandleDeleteUserRequest error: {}", e.what());
        nlohmann::json response{{"error", ERR_PARSE_ERROR}, {"message", "请求处理异常"}};
        session.Send(response.dump(), ID_DELETE_USER);
    }
    catch (...)
    {
        spdlog::error("[AuthService] HandleDeleteUserRequest unknown exception");
        nlohmann::json response{{"error", ERR_PARSE_ERROR}, {"message", "未知错误"}};
        session.Send(response.dump(), ID_DELETE_USER);
    }
    return true;
...
// ─── HandleExportDataRequest (ID_EXPORT_DATA 3002) ───

bool AuthService::HandleExportDataRequest(CSession &session, const std::string &body_data)
{
    DispatchGuard guard(session);
    ...
    try
    {
        ...
        {
            nlohmann::json response{{"error", ERR_INVALID_PARAM}, {"message", "参数无效"}};
            session.Send(response.dump(), ID_EXPORT_DATA);
            return true;
        }
        ...
        server->GetThreadPool().Enqueue(
            [safe_session, uid]()
            {
                ...
                auto messages = SQLiteMgr::Instance().Messages().GetAllMessagesByUid(uid);
                ...
                for (const auto &msg : messages)
                {
                    ...
                    item["type"] = msg.type;
                    item["image_id"] = msg.image_id;
                    item["recalled"] = msg.recalled;
                    item["edited"] = msg.edited;
                    export_data.push_back(item);
                }

                nlohmann::json response;
                response["error"] = 0;
                response["json_data"] = export_data.dump();
                safe_session->Send(response.dump(), ID_EXPORT_DATA);
                safe_session->ContinueReading();
            });
    ...
    {
        spdlog::error("[AuthService] HandleExportDataRequest error: {}", e.what());
        nlohmann::json response{{"error", ERR_PARSE_ERROR}, {"message", "请求处理异常"}};
        session.Send(response.dump(), ID_EXPORT_DATA);
    }
    catch (...)
    {
        spdlog::error("[AuthService] HandleExportDataRequest unknown exception");
        nlohmann::json response{{"error", ERR_PARSE_ERROR}, {"message", "未知错误"}};
        session.Send(response.dump(), ID_EXPORT_DATA);
    }
    return true;
