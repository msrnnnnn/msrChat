...
TEST_F(MessageOpsTest, SaveAndGetMessages)
{
    ...
    ASSERT_TRUE(SQLiteMgr::Instance().SaveMessage(msg1));
    ASSERT_TRUE(SQLiteMgr::Instance().SaveMessage(msg2));

    // before_time 大于所有消息时间戳，limit 足够
    auto results = SQLiteMgr::Instance().GetMessages(1, 2, 1700000099000LL, 50);
...
TEST_F(MessageOpsTest, GetMessages_Pagination)
{
    for (int i = 0; i < 5; i++)
    {
        ...
        ASSERT_TRUE(SQLiteMgr::Instance().SaveMessage(msg));
    }

    auto results = SQLiteMgr::Instance().GetMessages(10, 20, 1700000099000LL, 3);
...
TEST_F(MessageOpsTest, GetMessages_BeforeTime)
{
    ...
    ASSERT_TRUE(SQLiteMgr::Instance().SaveMessage(msg1));
    ASSERT_TRUE(SQLiteMgr::Instance().SaveMessage(msg2));
    ASSERT_TRUE(SQLiteMgr::Instance().SaveMessage(msg3));

    // before_time = 2500，应只返回 ts < 2500 的消息
    auto results = SQLiteMgr::Instance().GetMessages(30, 40, 2500LL, 50);
    ASSERT_EQ(results.size(), 2u);
...
TEST_F(MessageOpsTest, SaveMessage_InvalidUid)
{
    ...
    EXPECT_FALSE(SQLiteMgr::Instance().SaveMessage(msg));

    auto msg2 = MakeMsg(1, -1, "bad2", 2000LL);
    EXPECT_FALSE(SQLiteMgr::Instance().SaveMessage(msg2));
}
...
TEST_F(MessageOpsTest, Recall_MarkAndVerify)
{
    ...
    auto got = SQLiteMgr::Instance().GetMessageByTimestamp(1700000010000LL, 100);
...
TEST_F(MessageOpsTest, Edit_UpdateContent)
{
    ...
    auto got = SQLiteMgr::Instance().GetMessageByTimestamp(1700000030000LL, 100);
    ASSERT_TRUE(got.has_value());
...
TEST_F(MessageOpsTest, Recall_NonexistentMsg)
{
    ...
    auto got = SQLiteMgr::Instance().GetMessageByTimestamp(9999999999LL, 999);
...
TEST_F(MessageOpsTest, OfflineMessages)
{
    ...
    ASSERT_TRUE(SQLiteMgr::Instance().SaveOfflineMessage(off1));
    ASSERT_TRUE(SQLiteMgr::Instance().SaveOfflineMessage(off2));

    auto msgs = SQLiteMgr::Instance().GetOfflineMessages(60, 50);
    ASSERT_EQ(msgs.size(), 2u);
    EXPECT_EQ(msgs[0].content, "offline_1");
    EXPECT_EQ(msgs[1].content, "offline_2");

    // 清空后为空
    ASSERT_TRUE(SQLiteMgr::Instance().ClearOfflineMessages(60));
    auto after_clear = SQLiteMgr::Instance().GetOfflineMessages(60, 50);
...
TEST_F(MessageOpsTest, RecallNotify_Flow)
{
    ...
    ASSERT_TRUE(SQLiteMgr::Instance().EnqueueRecallNotify(target_uid, msg_ts, recall_uid, recall_ts, recalled_to));

    auto entries = SQLiteMgr::Instance().PopRecallNotifies(target_uid);
    ...
    // 清空后 Pop 返回空
    ASSERT_TRUE(SQLiteMgr::Instance().ClearRecallNotifies(target_uid));
    auto after_clear = SQLiteMgr::Instance().PopRecallNotifies(target_uid);
    EXPECT_TRUE(after_clear.empty());
}
