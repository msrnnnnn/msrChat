...
{
    auto result = SQLiteMgr::Instance().RegisterUser("alice", "client_hash_abc123", "alice@test.com");
...
{
    auto first = SQLiteMgr::Instance().RegisterUser("bob", "hash1", "bob@test.com");
    ASSERT_EQ(first.error, ERR_SUCCESS);

    auto second = SQLiteMgr::Instance().RegisterUser("bob", "hash2", "bob2@test.com");
...
{
    auto reg = SQLiteMgr::Instance().RegisterUser("charlie", "my_password_hash", "charlie@test.com");
    ASSERT_EQ(reg.error, ERR_SUCCESS);

    auto login = SQLiteMgr::Instance().LoginUser("charlie", "my_password_hash");
...
{
    auto reg = SQLiteMgr::Instance().RegisterUser("dave", "correct_hash", "dave@test.com");
    ASSERT_EQ(reg.error, ERR_SUCCESS);

    auto login = SQLiteMgr::Instance().LoginUser("dave", "wrong_hash");
    EXPECT_EQ(login.error, ERR_PASSWD_ERR);
}

TEST_F(AuthFlowTest, Login_NonexistentUser)
{
    auto login = SQLiteMgr::Instance().LoginUser("ghost", "any_hash");
...
{
    // 1. 注册用户
    auto reg = SQLiteMgr::Instance().RegisterUser("eve", "old_password_hash", "eve@test.com");
    ASSERT_EQ(reg.error, ERR_SUCCESS);

    // 2. 发送验证码
    int code = 0;
    ASSERT_TRUE(SQLiteMgr::Instance().SendVerifyCode("eve@test.com", code));
    EXPECT_GE(code, 100000);
    EXPECT_LE(code, 999999);

    // 3. 用正确验证码校验
    std::string code_str = std::to_string(code);
    EXPECT_EQ(SQLiteMgr::Instance().CheckVerifyCode("eve@test.com", code_str), ERR_SUCCESS);

    // 4. 用错误验证码校验
    EXPECT_EQ(SQLiteMgr::Instance().CheckVerifyCode("eve@test.com", "000000"), ERR_VERIFY_WRONG);

    // 5. 重置密码（使用正确验证码）
    int reset_result = SQLiteMgr::Instance().ResetPassword(
        "eve", "eve@test.com", code_str, "new_password_hash");
    EXPECT_EQ(reset_result, ERR_SUCCESS);

    // 6. 用新密码登录成功
    auto login_new = SQLiteMgr::Instance().LoginUser("eve", "new_password_hash");
    EXPECT_EQ(login_new.error, ERR_SUCCESS);

    // 7. 用旧密码登录失败
    auto login_old = SQLiteMgr::Instance().LoginUser("eve", "old_password_hash");
