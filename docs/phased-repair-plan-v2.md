...
| 序号 | 审查报告引用 | 修复方案 | 估算 |
|------|-------------|----------|------|
| 7C.1 | 容错#5 | 发送队列上限（per session，默认 1000 条积压，超出断开） | 0.5h |
| 7C.2 | 容错#9 | 写超时机制（与读超时对称，30s） | 1h |
| 7C.3 | 容错#13 | 消息幂等设计：基于 Phase 2.9 已添加的 client_msg_id 字段，在服务端实现去重逻辑 | 1.5h |
...
| 序号 | 审查报告引用 | 修复方案 | 估算 |
|------|-------------|----------|------|
| 7D.1 | 隐私#1 (高) | 账户注销功能（MVP）：服务端 API → username 改为 `已注销_XXXX`、清空 email → 返回成功。客户端仅需一个确认弹窗。**不做**：注销冷静期、原因收集、批量注销 | 2h |
| 7D.2 | 隐私#2 (高) | 数据导出功能（MVP）：服务端 API → 同步查询全部消息 → JSON 直出。**不做**：异步生成、下载链接、CSV 格式、进度条 | 2h |
