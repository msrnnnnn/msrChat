...
#include <QNetworkReply> // 补充：需要处理回包
#include <memory>        // 补充：std::shared_ptr 需要
#include <QMap>
#include <functional>
...
public:
    ...
    /**
     * @brief 注册业务模块处理器 (Registry Interface)
...
private slots:
     ...
     * * @param reply 也就是 QNetworkReply*，但在槽函数中通常通过 sender() 获取，或者使用 lambda 捕获
     * 通常我们是在 PostHttpRequest 里直接 connect lambda 或者 reply->deleteLater)
     */
...
signals:
     ...
     * @param err       错误码 (ERRORCODES::SUCCESS 或其他)
     * @param mod       模块 ID
     */
