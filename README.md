...
| **FileTransfer** | 文件传输核心，支持零拷贝和断点续传 |
...
| 指标 | 数值 | 说明 |
|------|------|------|
| **并发连接数** | 10,000+ | 单机支持万级并发（Epoll/IOCP） |
| **消息吞吐量** | 50,000+ msg/s | 纯内存处理，无外部IO |
| **文件传输** | 100+ MB/s | 零拷贝 + 分块并行 |
| **内存占用** | < 100MB | 空闲状态（不含缓冲） |
| **连接延迟** | < 10ms | 本地网络环境 |
| **心跳开销** | < 0.1% | 15秒间隔，消息体仅 4 字节 |
